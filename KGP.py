...
def build_kg_list(columns_list):
    ...
    while triger!='-1':
        ...
        #kg_list[i].add(item1)
        #kg_list[i].add(relation_name)
        #kg_list[i].add(item2)
        #kg_list[i]=tuple(kg_list[i])
        i=i+1
        triger=input("end? -1")   
    for i in kg_list:
        print(i)
...
def assig_index(df):
    itemtxt=open('../kgpreprocessor/item_index.txt','a',encoding='utf-8')
    relationtxt=open('../kgpreprocessor/relation_index.txt','a',encoding='utf-8')
    itemdict=dict()
    relationdict=dict()
    ii=0
    ri=0
...
def assig_business_index(df,itemdict,ii):
    itemtxt=open('../kgpreprocessor/item_index.txt','a',encoding='utf-8')
    usertxt=open('../kgpreprocessor/user_index.txt','a',encoding='utf-8')
...
while a == 1:
    ...
    filename =input("Please enter the name for the kg file")
    if filetype == "csv":
        output[0].to_csv('../kgpreprocessor/kg.csv')
        a=0
    elif filetype == "txt":
        output[0].to_csv('../kgpreprocessor/'+filename+ '.txt', sep = '\t',header = False, index = False)
        a=2
...
if d == "y" or d == "Y":
    ...
    rating_df=build_kg_list(rating_columns_list)
    rating_output=assig_business_index(rating_df,itemdict,ii)
    a=1
    while a == 1:
        filetype= input("Please enter the rating file type:(csv or txt) ")
        filename =input("Please enter the name for the rating file")
        if filetype == "csv":
            output[0].to_csv('../kgpreprocessor/'+filename+'.csv')
            a=0
        elif filetype == "txt":
            output[0].to_csv('../kgpreprocessor/'+filename+ '.txt', sep = '\t',header = False, index = False)
            a=2
